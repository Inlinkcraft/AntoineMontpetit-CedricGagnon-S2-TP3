...
public class Triangle extends Forme
{
		...
		if (validerCote(pCoteA) && validerCote(pCoteB) && validerCote(pCoteC))
		{
			...
			else
			{
				throw (new FormeException(
						"Côtés impossibles pour former un triangle"));
			}
		}
		else
		{
			throw (new FormeException(
					"Un ou plusieurs côtés invalides pour la construction"));
		}
	}
	...
	{
		// pow(x,0.5) = sqrt(x) laissez moi tranquille avec ça ok
		double p = ((double) calculerPerimetre()) / 2;
		return ((int) Math.pow((p * (p - coteA) * (p - coteB) * (p - coteC)),
				0.5));
	}
	...
	{
		return (super.toString() + " " + type + " " + coteA + ", " + coteB
				+ ", " + coteC);
	}
	...
	private int getNbrCoteEgaux()
	{
		...
		if (coteA == coteB && coteB == coteC)
		{
			...
			if (coteA == coteB || coteB == coteC || coteC == coteA)
			{
				total = 2;
			}
		return (total);
	...
	{
		// pow(a,2)+pow(b,2)==pow(c,2) est équivalent à
		// sqrt(pow(a,2)+pow(b,2))==c ???
		int[] temp = getCotesTries();
		return (Math.pow(temp[0], 2) + Math.pow(temp[1], 2) == Math.pow(temp[2],
				2));
	}

	/**
	 * Retourne le type du triangle entre Équilatéral, Rectangle, Isocèle et
	 * Scalène
	 * 
	...
	{
		// si a+b=c, le triangle est une ligne, ok, mais encore possible à
		// créer.
		return (pCoteA + pCoteB > pCoteC && pCoteA + pCoteC > pCoteB
				&& pCoteB + pCoteC > pCoteA);
	}
}